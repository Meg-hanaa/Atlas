...
from auth.setup import AUTH_SECRET, APP_PUBLIC_URL, User, auth_backend, get_user_manager
...
def _oauth_error_redirect(message: str) -> RedirectResponse:
    query = urlencode({"oauth_error": message[:500]})
    return RedirectResponse(
        f"{APP_PUBLIC_URL.rstrip('/')}?{query}",
...
if google_oauth_client is not None:
    ...
    async def google_oauth_callback(
        ...
        try:
            ...
            access_jwt = await strategy.write_token(user)
            login_response = await auth_backend.login(strategy, user)
            await user_manager.on_after_login(user, request, login_response)

            return RedirectResponse(
                google_post_login_url(access_jwt),
                status_code=status.HTTP_302_FOUND,
            )
